...
namespace MapsXF
{
    public class MapViewModel : BaseViewModel, IDisposable
    {
        ...
        public ICommand DownloadOfflineMapCommand => downloadOfflineMapCommand ?? (downloadOfflineMapCommand = new Command(async () =>
        {
            try
            {
                var geometry = await Editor.CreateAreaToDownloadAsync(Translate("Gen_Draw_Area_To_Download"));
                ...
                var shouldDownload = await ShowConfirmAsync(Translate("Download_Region"), Translate("Gen_Download_Offline_Map"));
                ...
                var permission = await PermissionService.Current.CheckPermissionAsync(Plugin.Permissions.Abstractions.Permission.Storage);
        ...
        public ICommand ChangeBasemapCommand => changeBasemapCommand ?? (changeBasemapCommand = new Command(async () =>
        {
            ...
            var res = await ShowActionSheetAsync(Translate("Select_Basemap"), Translate("Gen_Cancel"), null, basemapTypes);

            if (string.IsNullOrEmpty(res) || res?.Equals(Translate("Gen_Cancel")) == true)
            {
                return;
            }
